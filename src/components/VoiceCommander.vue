...
// Variables per la Tasca 3 (Snackbar)
const showSnackbar = ref(false);
const snackbarText = ref("");

// Reacció al text entès
watch(transcript, (newText) => {
  if (!newText) return;
  const command = newText.toLowerCase().trim();
  
  // Reiniciem l'snackbar per defecte si la comanda és vàlida
  ...
  // TASCA 2: MODE CLAR
    ...
    uiMessage.value = "Mode clar activat ☀️";
    statusColor.value = "primary";
  }
  // TASCA 3: COMANDA NO RECONEGUDA (SNACKBAR)
  else {
